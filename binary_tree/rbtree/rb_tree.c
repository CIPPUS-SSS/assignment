 ...
 *
 */
rbtree_node move_red_right(node h){
...
rbtree _delete_max(node h){
	...
	h.right = delete_max(h.left);
	return fixup(h);
}

int delete(rbtree tree,key){
	return -1;
}

rbtree_node  _delete(node h,key){
	return NULL;
}
